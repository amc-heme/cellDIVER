...
cellDIVER can be deployed with Docker to facilitate deployment on a server, or to facilitate sharing cellDIVER instances with your data with collaborators. Deployment on docker is not required to run cellDIVER. For general instructions on setting up an app instance with your data, see the [Dataset Setup Walkthrough](https://amc-heme.github.io/scExploreR/articles/dataset_setup_walkthrough.html) article.
...
RUN R -e "remotes::install_github('amc-heme/scExploreR');  if (!library(cellDIVER, logical.return=T)) quit(status=10)"
...
-   [cellDIVER GitHub Repository](https://github.com/amc-heme/scExploreR)
