---
title: "Running cellDIVER with Docker"
...
cellDIVER can be deployed with Docker to facilitate deployment on a server, or to facilitate sharing cellDIVER instances with your data with collaborators. Deployment on docker is not required to run cellDIVER. For general instructions on setting up an app instance with your data, see the [Dataset Setup Walkthrough](https://amc-heme.github.io/cellDIVER/articles/dataset_setup_walkthrough.html) article.

This guide provides step-by-step instructions for setting up and running the cellDIVER Shiny app using Docker. We'll cover building the Docker image, understanding the file structure, configuring the application, and running the app.
...
<h4>**cellDIVER_docker Repository**</h4>
...
-   **config.yaml**: Configuration file specifying datasets and settings for cellDIVER .
-   **object-config.yaml**: Configuration for the specific data object built using the cellDIVER Config App.
...
The `Dockerfile` specifies the environment and dependencies required to run cellDIVER.
...
The `config.yaml` file specifies settings for cellDIVER, and the paths of datasets to load in the app. This is the app config file (rather than the object config file). For more information on how to generate this file, see the [Dataset Setup Walkthrough](https://amc-heme.github.io/cellDIVER/articles/dataset_setup_walkthrough.html#create-an-app-config-file) article.
...
# Step 5: Set up cellDIVER Script {#step-5}

The `app.R` script defines how cellDIVER will be ran. This file should be an R script with a call to `run_cellDIVER`, with the `browser_config` parameter set to the path of the app config file created in step 4.
...
-   `run_cellDIVER`: Launches the cellDIVER app with the specified configuration.
...
Once the container is running, open your web browser and navigate to `http://localhost:3838`. You should see the cellDIVER interface.
...
-   [Dataset Setup Walkthrough](https://amc-heme.github.io/cellDIVER/articles/dataset_setup_walkthrough.html)
