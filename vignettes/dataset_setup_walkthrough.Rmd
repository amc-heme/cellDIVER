...
By the end of this example, you will be able to set up an cellDIVER instance with both datasets<!--, and you will be able to reproduce some of the figures in the paper using the plots tab. This vignette will also show how to identify differentially expressed genes between groups of cells using a wilcoxon rank sum-->.
...
To host a dataset in cellDIVER, you must indicate which metadata variables, reductions, and modalities to include in the app. Whether you are setting up the app for end users or yourself, careful selection of the most relevant data is key to maximizing the effectiveness of cellDIVER for your dataset.
...
In this section, we will load the "Combined Samples" and "SCLC epithelial cells" objects and explore relevant data to include in the cellDIVER instance.
...
Other datasets may include surface protein (ADT) data, computed gene signatures per cell, or other data. **For anndata objects, alternate modalities must be stored in `obsm` to be accessible to cellDIVER** (with the exception of `X`, of course). Seurat and SingleCellExperiment objects have defined slots for modalities: `@assays` in Seurat objects, and "alternate experiments" for SingleCellExperiment objects. For these object types, cellDIVER will use these slots to access modality information. As long as feature expression data for each modality is stored using the convention for Seurat and SingleCellExperiment objects, no additional action is needed for cellDIVER to find this data.
:::
...
<!-- Don't forget to mention in the full object section that only categorical            metadata can be added to cellDIVER -->
...
Alternate modalities and reductions should be stored in `uns`, under `cellDIVER_assays` and `cellDIVER_reductions`, respectively. This is done below in python, via reticulate, for both the full object and the SCLC object.
...
The final step before launching cellDIVER is to create an app config file. The app config file specifies the paths of all objects used in the app and their respective config files, and it also defines app-wide settings. An example app config file is given below. 
...
If you only have one object, you don't need to make an app config file. You can simply run `run_cellDIVER`, using the `object_path` and `config_path` parameters instead of `browser_config`.
