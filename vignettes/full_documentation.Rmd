...
Differential gene expression (DGE) analysis compares groups and returns a table of genes that differ in expression between them. scExploreR provides two statistical methods. The cell-level Wilcoxon rank-sum method uses [presto](https://www.biorxiv.org/content/10.1101/653253v1.full) on normalized expression and also reports an area under the receiver operator curve (auROC). For replicated standard two-group comparisons, the edgeR pseudobulk method aggregates raw counts by biological sample, applies TMM normalization, and uses edgeR's quasi-likelihood framework.
...
-   For differential expression based on categorical metadata, choose Wilcoxon or edgeR pseudobulk. edgeR can run one contrast, all pairwise contrasts, selected groups versus the rest, or selected groups versus a reference. Marker identification and feature-expression-threshold comparisons use Wilcoxon.
...
**Wilcoxon rank-sum** is a fast exploratory test on cell-level normalized expression. Because cells from the same biological sample are not independent replicates, its p-values can be overly optimistic when the scientific question concerns differences across biological samples.

**edgeR pseudobulk** is available for comparisons defined by categorical metadata. For sample-level conditions it aggregates raw integer counts by biological sample. For cell-level groups such as clusters it aggregates by biological sample and group, retains adequately represented paired samples, and fits a sample-blocked model. It then performs TMM normalization, expression filtering, dispersion estimation, and quasi-likelihood testing. The dataset configuration must identify the biological-sample metadata field and genes assay. Each retained pseudobulk must contain at least the selected number of cells (10 by default). At least two retained biological samples are required; three or more are recommended.

The edgeR contrast control supports a single contrast, every pairwise comparison among the chosen groups, each chosen group versus all other cells, or each chosen group versus a reference group. Leaving "Groups to Test" empty uses every observed group in the current subset. edgeR is not available for marker identification, feature-expression-threshold groups, additional batch covariates, or arbitrary model formulas. The app does not silently substitute normalized expression when raw counts are missing and does not fall back to Wilcoxon when edgeR validation fails.

Group 1 is always the numerator of an edgeR contrast. For one-versus-rest analyses, the named cluster is Group 1; for reference analyses, the reference is Group 2. Positive log2 fold changes therefore indicate higher expression in Group 1.
...
When checked, the table returned will only show genes with a positive log2 fold change. For edgeR, this means genes higher in Group 1 than Group 2; clear the checkbox to retain genes higher in Group 2 as negative fold changes. For Wilcoxon marker results, positive genes are those upregulated in the displayed group or marker class.
...
The "test summary" section displays the submitted statistical method and the groups used. For edgeR it reports retained biological-sample counts and excluded pseudobulk profiles for each contrast. The number of cells in the analyzed subset and in each DGE group or marker class is also shown.
...
-   Average expression: the average expression of the gene in the group or class listed. This is reported for Wilcoxon results.

-   logCPM: the average log2 counts per million across pseudobulk libraries. This is reported for edgeR results.
...
-   AUC: the area under the receiver operator curve for the gene, which serves as a measure of how well the gene serves as a marker for the indicated group/class. This is reported for Wilcoxon results.

-   pval: the unadjusted p-value. For edgeR this is produced by the quasi-likelihood F-test.
...
-   n_samples_group_1 and n_samples_group_2: the retained biological-sample counts used by edgeR.

-   method: the edgeR workflow used to produce the row.
