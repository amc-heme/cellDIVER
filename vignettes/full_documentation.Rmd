...
-   For a standard two-group comparison, choose Wilcoxon or edgeR pseudobulk. Marker identification and feature-expression-threshold comparisons use Wilcoxon.
...
**edgeR pseudobulk** is available for standard two-group comparisons defined by categorical metadata. It sums raw integer counts for cells from the same biological sample and condition, then performs TMM normalization, expression filtering, dispersion estimation, and a quasi-likelihood test. The dataset configuration must identify the biological-sample metadata field and genes assay. Each retained pseudobulk must contain at least the selected number of cells (10 by default). At least two retained biological samples per group are required; three or more per group are recommended.

edgeR is not currently available for marker identification, feature-expression-threshold groups, paired or repeated-measures designs, batch covariates, or arbitrary model formulas. The app does not silently substitute normalized expression when raw counts are missing and does not fall back to Wilcoxon when edgeR validation fails.

Group 1 is the numerator of the edgeR contrast. Positive log2 fold changes therefore indicate higher expression in Group 1 than Group 2.
...
The "test summary" section displays the submitted statistical method and the groups used. For edgeR it also reports the retained biological-sample count in each group and the number of pseudobulk samples excluded by the minimum-cell filter. The number of cells in the analyzed subset and in each DGE group or marker class is also shown.
