# Running cellDIVER with Docker

cellDIVER can be deployed with Docker to facilitate deployment on a
server, or to facilitate sharing cellDIVER instances with your data with
collaborators. Deployment on docker is not required to run cellDIVER.
For general instructions on setting up an app instance with your data,
see the [Dataset Setup
...
the cellDIVER Shiny app using Docker. We’ll cover building the Docker
image, understanding the file structure, configuring the application,
and running the app.

#### **cellDIVER_docker Repository**
...
- **app.R**: Main R script to run the cellDIVER app.
...
- **object-config.yaml**: Configuration for the specific data object
  built using the cellDIVER Config App.
...
RUN R -e "remotes::install_github('amc-heme/cellDIVER');  if (!library(cellDIVER, logical.return=T)) quit(status=10)"
...
The `shiny-server.conf` file sets up the Shiny server to host cellDIVER.
...
Walkthrough](https://amc-heme.github.io/cellDIVER/articles/dataset_setup_walkthrough.html#launching-the-config-app)
Article.
...
The `config.yaml` file specifies settings for cellDIVER, and the paths
...
Walkthrough](https://amc-heme.github.io/cellDIVER/articles/dataset_setup_walkthrough.html#create-an-app-config-file)
article.
...
## Step 5: Set up cellDIVER Script

The `app.R` script defines how cellDIVER will be ran. This file should
be an R script with a call to `run_cellDIVER`, with the `browser_config`
parameter set to the path of the app config file created in step 4.
...
- `setwd`: Sets the working directory to where data and configuration
  files are located.
- `run_cellDIVER`: Launches the cellDIVER app with the specified
  configuration.
...
- `-t celldiver` : Tags the image with the name celldiver.
...
`http://localhost:3838`. You should see the cellDIVER interface.
...
``` bash
docker run -it --entrypoint /bin/bash -p 3838:3838 celldiver
```

## References

- [cellDIVER GitHub Repository](https://github.com/amc-heme/cellDIVER)
