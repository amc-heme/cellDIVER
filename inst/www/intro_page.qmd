...
Use the differential expression tab to identify genes that are upregulated or downregulated between groups in the dataset. Standard two-group comparisons can use either a cell-level Wilcoxon rank-sum test or an edgeR pseudobulk test. Marker identification and groups defined by a feature-expression threshold use Wilcoxon.
...
**Step 2: Choose a statistical method**

- **Wilcoxon rank-sum:** Uses cell-level normalized expression. This fast exploratory method treats cells as observations.
- **edgeR pseudobulk:** Uses raw counts summed within each biological sample, followed by TMM normalization and a quasi-likelihood test. Use this method when biological replicates are available and inference across samples is the goal.

edgeR requires a configured biological-sample metadata field, at least two retained samples per group, and raw integer counts in the genes assay. Three or more samples per group are recommended. Pseudobulk samples with fewer than the selected minimum number of cells (10 by default) are excluded. Group 1 is the numerator, so a positive log2 fold change means higher expression in Group 1 than Group 2.
